...
class GraphSparse(Graph):
    ...
    @abstractmethod
    def vertex_weight_et(self):
        return silearn.scatter_sum(self._p, self.edges[0][:, 1])
...
class GraphDense(Graph):
    ...
    @property
    def vertex_weight_es(self):
        return self.adj.sum(dim = 1)

    @property
    @abstractmethod
    def vertex_weight_et(self):
        return self.adj.sum(dim = 0)
